    ...
    const fetchBlogPosts = async () => {
      try {
        const response = await axios.get(
          'https://medium.com/@afimiafidelis95/blog'
        );
      ...
      <div className="max-w-custom mx-auto px-5 my-28">
        <section className="text-left lg:text-left text-newDark dark:text-white">
          ...
          >
            ...
            <p>
              <a href="http://medium.com/@afimiafidelis95">Read more</a>
            </p>
          </motion.article>
          <div className="grid gap-12 grid-cols-1">
            {blogPosts.map((item, index) => (
              <article
                className="grid gap-0 md:grid-cols-2 place-content-center md:gap-12"
                key={index}
              >
                <motion.figure
                  initial="hidden"
                ...
                >
                  <img
                    src={extractImgSrc(item.content)}
                    alt="try"
                    className=""
                  />
                  <a href={item.link}>
